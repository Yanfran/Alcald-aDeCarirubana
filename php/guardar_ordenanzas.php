...
$conexion=conexion();
if(getVerificar($conexion,"ordenanzas")){
...
$titulo=$_POST['titulo'];
    ...
    if($_FILES['documento']['type']=='application/pdf'){            
        ...
        $sql="SELECT * FROM ordenanzas WHERE titulo='$titulo' and status!='2'";
        ...
        if(is_countable($fila)>0){
            ...
            if (isset($_FILES['documento']) && !empty($_FILES['documento'])) {            
                ...
                $sql2="INSERT INTO ordenanzas(titulo,ruta,status) values ('$titulo','$nombreFinal',1)";        

...
// var_dump($documento);
    ...
    if (isset($_FILES['documento']) && !empty($_FILES['documento'])) {
            ...
            if($_FILES['documento']['type']=='application/pdf'){
                ...
                $guardar="UPDATE entes_descentralizados_documentos SET titulo='$titulo', ruta='$nombreFinal' WHERE id='$id'";                        
            }

    } else {
        $guardar="UPDATE entes_descentralizados_documentos SET titulo='$titulo' WHERE id='$id'";
    }
    ...
    if ($guardado) {
        if(isset($_FILES['documento'])){
            unlink('../img/inicio/entes_descentralizados_ducumentos/'.$antigua);            
    }
        echo 1;
    }else{
        echo 2;

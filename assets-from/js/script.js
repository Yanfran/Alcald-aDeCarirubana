(function($) {
	...
	// Popup Emergente	
	if($('#popup-emergente').length){
		
		//Show Popup
		$('.emergente-toggler').on('click', function() {
			$('#popup-emergente').addClass('popup-visible');
		});
		$(document).keydown(function(e){
	        if(e.keyCode === 27) {
	            $('#popup-emergente').removeClass('popup-visible');
	        }
	    });
		//Hide Popup
		$('.close-search,.popup-emergente .overlay-layer').on('click', function() {
			$('#popup-emergente').removeClass('popup-visible');
		});
	}
